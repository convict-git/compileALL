...
int main(){
	...
	string strinp;
	system("ls *.cpp > ls");//creates a list of all c files and stores it into a file
	int DONE=0, lineN=1;
	ifstream read("ls");
	while( read >> strinp){//reads all your c file names from the file
		cout << lineN++ << "  Compiling " << strinp<< endl << " g++ -std=c++11 -O2 " << strinp << " -o " << strout(strinp);
		ostringstream gcc;
		gcc << "g++ -std=c++11 -O2 "<< strinp << " -o " << strout(strinp);//compilation with optimisations
